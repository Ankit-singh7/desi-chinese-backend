...
/* Models */
const stockManagerModel = mongoose.model('stockManager')
    ...
    stockManagerModel.find()
        ...
        .exec((err, result) => {
            if (err) {
                ...
                let smList = result.slice(startIndex,endIndex)
                let newResult = {total:total,result: smList}
...
let getSingleUser = (req, res) => {
    stockManagerModel.findOne({ 'stockManagerId': req.params.id })
        .select('-__v -_id')
        .lean()
        .exec((err, result) => {
            if (err) {
...
let signUpFunction = (req, res) => {
    ...
    let createUser = () => { 
        return new Promise((resolve, reject) => {
            stockManagerModel.findOne({ email: req.body.email })
...
let resetPasswordFunction = (req,res) => {
    stockManagerModel.find({'email':req.body.email})
    ...
    .exec((err, result) => {
        if (err) {
            ...
            if(req.body.oldPassword === result[0].password) {
                ...
                stockManagerModel.updateOne({'email':req.body.email},options)
                .select('-password')
                .exec((err,result) => {
                    if(err) {
                        console.log(err)
